...
struct VTTModal: View {
    @Binding var isPresented: Bool
    @State private var dragOffset: CGSize = .zero
    @State private var yearlyStats: [YearlyVTTStats] = []
    
    struct YearlyVTTStats: Identifiable {
        let id = UUID()
        let year: Int
        ...
        ZStack {
            ...
            VStack(spacing: 20) {
                HStack {
                    ...
                    Image(systemName: "bicycle").font(.system(size: 30)).foregroundColor(.white)
                    Text("VTT").font(.system(size: 28, weight: .bold)).foregroundColor(.white)
                    Spacer()
                ...
                VStack(spacing: 15) {
                    Text("Statistiques VTT").font(.system(size: 20, weight: .semibold)).foregroundColor(.white.opacity(0.9))
                    
                    if !yearlyStats.isEmpty {
                        Chart(yearlyStats) { stat in
                            BarMark(x: .value("Année", String(stat.year)), y: .value("Kilomètres", stat.kilometers))
                            // ✅ AJOUT 2 : jaune pour l'année en cours (mint trop proche de cyan), mint pour les autres
                            .foregroundStyle(
                                stat.year == currentYear
                                    ...
                                    : LinearGradient(colors: [Color.mint.opacity(0.8), Color.mint.opacity(0.5)], startPoint: .top, endPoint: .bottom)
                            )
                            .cornerRadius(4)
                            // ✅ AJOUT 3
                        ...
                        ZStack {
                            ...
                            VStack(spacing: 12) {
                                Image(systemName: "figure.outdoor.cycle").font(.system(size: 60)).foregroundColor(.brown.opacity(0.8))
                                Text("Aucune donnée VTT disponible").font(.system(size: 16)).foregroundColor(.white.opacity(0.7))
                            }
                            .padding()
                        }
                        .frame(height: 200)
            ...
            .background(LinearGradient(gradient: Gradient(colors: [Color.mint.opacity(0.6), Color.mint.opacity(0.3)]), startPoint: .topLeading, endPoint: .bottomTrailing))
    ...
    private func loadVTTStats() {
        ...
        yearlyStats = yearlyData.map { YearlyVTTStats(year: $0.key, kilometers: $0.value) }.sorted { $0.year < $1.year }
    }
}

#Preview { VTTModal(isPresented: .constant(true)) }
