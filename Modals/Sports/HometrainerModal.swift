...
struct HomeTrainerModal: View {
    @Binding var isPresented: Bool
    @State private var dragOffset: CGSize = .zero
    @State private var yearlyStats: [YearlyHomeTrainerStats] = []
    
    struct YearlyHomeTrainerStats: Identifiable {
        let id = UUID()
        let year: Int
    ...
    var body: some View {
        ZStack {
                ...
                VStack(spacing: 15) {
                    ...
                    if !yearlyStats.isEmpty {
                        Chart(yearlyStats) { stat in
                            ...
                            // ✅ AJOUT 2 : cyan pour l'année en cours, purple pour les autres
                            .foregroundStyle(
                                stat.year == currentYear
                                    ? LinearGradient(colors: [Color.cyan.opacity(0.9), Color.cyan.opacity(0.5)], startPoint: .top, endPoint: .bottom)
                                    : LinearGradient(colors: [Color.purple.opacity(0.8), Color.purple.opacity(0.5)], startPoint: .top, endPoint: .bottom)
                            )
                            .cornerRadius(4)
                            // ✅ AJOUT 3
                        ...
                        ZStack {
                            ...
                            VStack(spacing: 12) {
                                Image(systemName: "figure.indoor.cycle").font(.system(size: 60)).foregroundColor(.purple.opacity(0.8))
                                Text("Aucune donnée de home trainer disponible").font(.system(size: 16)).foregroundColor(.white.opacity(0.7))
                            }
                            .padding()
                        }
                        .frame(height: 200)
            ...
            .background(LinearGradient(gradient: Gradient(colors: [Color.purple.opacity(0.6), Color.purple.opacity(0.3)]), startPoint: .topLeading, endPoint: .bottomTrailing))
            .cornerRadius(20)
            .shadow(color: Color.black.opacity(0.3), radius: 20, x: 0, y: 10)
            .offset(x: dragOffset.width, y: dragOffset.height)
            .gesture(DragGesture().onChanged { value in dragOffset = value.translation })
        }
        .onAppear { loadHomeTrainerStats() }
    }
    
    private func loadHomeTrainerStats() {
        let allTrainings = TrainingDataManager.shared.loadTrainings()
        let homeTrainerTrainings = allTrainings.filter { $0.type == "Home trainer" }
        // ✅ AJOUT 5
        let currentYearValue = Calendar.current.component(.year, from: Date())
        var yearlyData: [Int: Double] = [:]
        for training in homeTrainerTrainings {
        ...
        yearlyStats = yearlyData.map { YearlyHomeTrainerStats(year: $0.key, kilometers: $0.value) }.sorted { $0.year < $1.year }
    }
}

#Preview { HomeTrainerModal(isPresented: .constant(true)) }
