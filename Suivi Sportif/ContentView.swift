...
struct ContentView: View {
        ...
        ZStack {
            ...
            VStack(spacing: 10) {
                ...
                if showOrangeRectangle {
                    VStack(spacing: 15) {
                        ZStack {
                            Rectangle().fill(Color.orange.opacity(0.35)).cornerRadius(16)
                            VStack(spacing: 12) {
                                HStack {
                                    Image(systemName: "gift.fill").font(.system(size: 20)).foregroundColor(.yellow)
                                    Text("Fête du jour").font(.system(size: 18, weight: .bold)).foregroundColor(.white)
                                }
                                if let info = todayInfo {
                                    Text(info.fete).font(.system(size: 32, weight: .bold)).foregroundColor(.white).padding(.horizontal)
                                } else {
                                    VStack(spacing: 8) {
                                        Image(systemName: "calendar.badge.exclamationmark").font(.system(size: 20)).foregroundColor(.white.opacity(0.5))
                                        Text("Aucune information disponible").font(.system(size: 13)).foregroundColor(.white.opacity(0.7)).italic()
                                    }
                                }
                            }.padding()
                        }
                        ...
                        ZStack {
                            ...
                            VStack(spacing: 6) {
                                HStack {
                                    Image(systemName: "quote.opening").font(.system(size: 18)).foregroundColor(.yellow)
                                    Text("Dicton du jour").font(.system(size: 18, weight: .bold)).foregroundColor(.white)
                                }
                                if let info = todayInfo, !info.dicton.isEmpty && info.dicton != "Aucun dicton pour aujourd'hui" {
                                    HStack(alignment: .top, spacing: 8) {
                                        Text("\"").font(.system(size: 24, weight: .bold)).foregroundColor(.yellow).offset(y: -5)
                                        Text(info.dicton).font(.system(size: 18, weight: .medium)).foregroundColor(.white.opacity(0.95)).multilineTextAlignment(.center).lineLimit(5).fixedSize(horizontal: false, vertical: true)
                                        Text("\"").font(.system(size: 24, weight: .bold)).foregroundColor(.yellow).offset(y: 10)
                                    }.padding(.horizontal, 20)
                                } else {
                                    VStack(spacing: 8) {
                                        Image(systemName: "text.quote").font(.system(size: 20)).foregroundColor(.white.opacity(0.5))
                                        Text("Pas de dicton pour aujourd'hui").font(.system(size: 13)).foregroundColor(.white.opacity(0.7)).italic()
                                    }
                                }
                            }.padding()
                        }
                ...
                if showPurpleRectangle {
                    ZStack {
                        Rectangle().fill(Color.purple.opacity(0.35)).cornerRadius(16)
                    }
    ...
    private var scrollingTextView: some View {
        ZStack {
            GeometryReader { geo in
                ...
                Text(repeatedText)
                    .font(.system(size: 18, weight: .semibold)).foregroundColor(.yellow)
    ...
    private var scrollingTextViewRight: some View {
        ZStack {
            ...
            GeometryReader { geo in
                ...
                Text(repeatedText)
                    .font(.system(size: 18, weight: .semibold)).foregroundColor(.cyan)
                    .lineLimit(1).fixedSize()
    ...
    private var titleView: some View {
        HStack(spacing: 15) {
            if let gifURL = Bundle.main.url(forResource: "coureur", withExtension: "gif") {
                AnimatedImage(url: gifURL).resizable().scaledToFit()
                    .frame(width: 120, height: 120).offset(x: 120, y: 15).zIndex(10)
            } else {
                Image(systemName: "figure.run.circle.fill").font(.system(size: 70))
                    .foregroundColor(.white).offset(x: 10, y: 15).zIndex(10)
            }
            Text("Suivi      Entrainements")
                .font(.system(size: 46, weight: .bold, design: .rounded))
                .foregroundColor(.white).offset(x: -90)
        }
    ...
    private var imageZonesContainer: some View {
        ZStack(alignment: .topLeading) {
            ...
            HStack(alignment: .top, spacing: 8) {
                ZStack {
                    Rectangle().fill(Color.green.opacity(0.6)).cornerRadius(6)
                    Text("\(Calendar.current.component(.year, from: Date()))")
                        .font(.system(size: 12, weight: .bold)).foregroundColor(.white)
                }
                .frame(width: 60, height: 35).offset(y: -47)
                ...
                HStack(alignment: .bottom, spacing: 0) {
                    ForEach(0..<12, id: \.self) { index in
                        let imageWidth: CGFloat = {
                            ...
                            case "triathlon":    return imageZones[index].width + 4
                        ...
                        let imageHeight: CGFloat = {
                            ...
                            case "Home trainer": return imageZones[index].height + 2
                            ...
                            Button(action: {
                                ...
                                Group {
                                    if NSImage(named: imageZones[index].imageName) != nil {
                                        ...
                                        VStack(spacing: 4) {
                                            Image(systemName: "photo.badge.plus").font(.system(size: 20)).foregroundColor(.white.opacity(0.5))
                                            Text("Ajouter").font(.system(size: 8)).foregroundColor(.white.opacity(0.5))
                                        }
                                        .frame(width: imageZones[index].width, height: imageZones[index].height)
                                    }
                                }
    ...
    private func yearBox(_ year: Int?, color: Color) -> some View {
        ZStack {
            ...
            if let year { Text("\(year)").font(.system(size: 12, weight: .bold)).foregroundColor(.white) }
        }
                    ...
                    ForEach(0..<8, id: \.self) { index in
                        ZStack {
                            ...
                            Text(headerRowValues[index])
                                .font(.system(size: headerTextZones[index].fontSize,
                                    weight: (index == 3 || index == 7) ? headerKmFontWeight : .semibold))
                                .foregroundColor((index == 3 || index == 7)
                                    ? headerKmTextColor.opacity(headerKmTextOpacity) : .white)
                                .multilineTextAlignment(.center).padding(4)
                        }
        ...
        VStack(spacing: 10) {
            ...
            Button(action: { currentDestination = .trainingPlanMortone }) {
                menuButton(
                    ...
                    bg: currentDestination == .trainingPlanMortone ? Color.pink.opacity(0.6) : Color.pink.opacity(0.25)
                )
            }
            .buttonStyle(.plain)
            // ──────────────────────────────────────────────────────
