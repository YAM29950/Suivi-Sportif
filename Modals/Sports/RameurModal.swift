...
struct RameurModal: View {
    @Binding var isPresented: Bool
    @State private var dragOffset: CGSize = .zero
    @State private var yearlyStats: [YearlyRameurStats] = []
    
    struct YearlyRameurStats: Identifiable {
        let id = UUID()
        let year: Int
                ...
                VStack(spacing: 15) {
                    ...
                    if !yearlyStats.isEmpty {
                        Chart(yearlyStats) { stat in
                            ...
                            // ✅ AJOUT 2 : cyan pour l'année en cours, indigo pour les autres
                            .foregroundStyle(
                                stat.year == currentYear
                                    ...
                                    : LinearGradient(
                                        colors: [Color.indigo.opacity(0.8), Color.indigo.opacity(0.5)],
                                        startPoint: .top,
                                        endPoint: .bottom
                                      )
                            )
                        ...
                        // ✅ AJOUT 4 : label de l'année en cours en cyan gras sur l'axe X
                        .chartXAxis {
                            AxisMarks { value in
                                AxisValueLabel()
                                    .foregroundStyle(
                        ...
                        ZStack {
                            ...
                            VStack(spacing: 12) {
                                ...
                                Text("Aucune donnée de rameur disponible")
                                    .font(.system(size: 16))
    ...
    private func loadRameurStats() {
        // Charger tous les entraînements
        let allTrainings = TrainingDataManager.shared.loadTrainings()
        
        // Filtrer uniquement les rameurs
        let rameurTrainings = allTrainings.filter { $0.type == "Rameur" }
        ...
        for training in rameurTrainings {
        ...
        yearlyStats = yearlyData.map { YearlyRameurStats(year: $0.key, kilometers: $0.value) }
            .sorted { $0.year < $1.year }
    }
}

#Preview {
    RameurModal(isPresented: .constant(true))
}
