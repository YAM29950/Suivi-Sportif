...
struct PiscineModal: View {
    @Binding var isPresented: Bool
    @State private var dragOffset: CGSize = .zero
    @State private var yearlyStats: [YearlyPiscineStats] = []
    
    struct YearlyPiscineStats: Identifiable {
        let id = UUID()
        let year: Int
        ...
        ZStack {
            ...
            VStack(spacing: 20) {
                HStack {
                    ...
                    Image(systemName: "figure.pool.swim").font(.system(size: 30)).foregroundColor(.white)
                    Text("Piscine").font(.system(size: 28, weight: .bold)).foregroundColor(.white)
                    Spacer()
                ...
                VStack(spacing: 15) {
                    Text("Statistiques Piscine").font(.system(size: 20, weight: .semibold)).foregroundColor(.white.opacity(0.9))
                    
                    if !yearlyStats.isEmpty {
                        Chart(yearlyStats) { stat in
                            BarMark(x: .value("Année", String(stat.year)), y: .value("Kilomètres", stat.kilometers))
                            // ✅ AJOUT 2 : cyan pour l'année en cours, blue pour les autres
                            .foregroundStyle(
                                stat.year == currentYear
                                    ? LinearGradient(colors: [Color.cyan.opacity(0.9), Color.cyan.opacity(0.5)], startPoint: .top, endPoint: .bottom)
                                    : LinearGradient(colors: [Color.blue.opacity(0.8), Color.blue.opacity(0.5)], startPoint: .top, endPoint: .bottom)
                            )
                            .cornerRadius(4)
                            // ✅ AJOUT 3
                        ...
                        ZStack {
                            ...
                            VStack(spacing: 12) {
                                Image(systemName: "figure.pool.swim").font(.system(size: 60)).foregroundColor(.blue.opacity(0.8))
                                Text("Aucune donnée de piscine disponible").font(.system(size: 16)).foregroundColor(.white.opacity(0.7))
                            }
                            .padding()
                        }
                        .frame(height: 200)
    ...
    private func loadPiscineStats() {
        ...
        yearlyStats = yearlyData.map { YearlyPiscineStats(year: $0.key, kilometers: $0.value) }.sorted { $0.year < $1.year }
    }
}

#Preview { PiscineModal(isPresented: .constant(true)) }
