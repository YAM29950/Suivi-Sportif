...
struct RouteModal: View {
    ...
    @State private var yearlyStats: [YearlyRouteStats] = []
    
    struct YearlyRouteStats: Identifiable {
        let id = UUID()
        let year: Int
        ...
        ZStack {
            ...
            VStack(spacing: 20) {
                HStack {
                    ...
                    Text("Vélo Route").font(.system(size: 28, weight: .bold)).foregroundColor(.white)
                    Spacer()
                ...
                VStack(spacing: 15) {
                    Text("Statistiques Vélo Route").font(.system(size: 20, weight: .semibold)).foregroundColor(.white.opacity(0.9))
                    
                    if !yearlyStats.isEmpty {
                        Chart(yearlyStats) { stat in
                            BarMark(x: .value("Année", String(stat.year)), y: .value("Kilomètres", stat.kilometers))
                            // ✅ AJOUT 2 : cyan pour l'année en cours, brown pour les autres
                            .foregroundStyle(
                                stat.year == currentYear
                                    ? LinearGradient(colors: [Color.cyan.opacity(0.9), Color.cyan.opacity(0.5)], startPoint: .top, endPoint: .bottom)
                                    : LinearGradient(colors: [Color.brown.opacity(0.8), Color.brown.opacity(0.5)], startPoint: .top, endPoint: .bottom)
                            )
                            .cornerRadius(4)
                            // ✅ AJOUT 3
                        ...
                        ZStack {
                            ...
                            VStack(spacing: 12) {
                                Image(systemName: "bicycle.circle.fill").font(.system(size: 60)).foregroundColor(.mint.opacity(0.8))
                                Text("Aucune donnée de vélo route disponible").font(.system(size: 16)).foregroundColor(.white.opacity(0.7))
                            }
                            .padding()
                        }
                        .frame(height: 200)
            ...
            .background(LinearGradient(gradient: Gradient(colors: [Color.brown.opacity(0.6), Color.brown.opacity(0.3)]), startPoint: .topLeading, endPoint: .bottomTrailing))
    ...
    private func loadRouteStats() {
        ...
        yearlyStats = yearlyData.map { YearlyRouteStats(year: $0.key, kilometers: $0.value) }.sorted { $0.year < $1.year }
    }
}

#Preview { RouteModal(isPresented: .constant(true)) }
